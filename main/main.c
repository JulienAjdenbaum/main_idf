...
void play_wav(const char *path) {
    ...
    size_t bytes_read;
    while (1) {
        uint8_t *buffer = malloc(2048);  // Allocate NEW buffer each iteration
        bytes_read = fread(buffer, 1, 2048, file);
        if (bytes_read == 0) {
            free(buffer);
            break;
        }
        // // Ensure even number of samples for I2S
        // if (bytes_read % 2 != 0) {
        //     bytes_read -= 1;  // Discard last odd byte
        // }
